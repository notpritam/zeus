...
type BrowserViewTab = 'snapshot' | 'screenshot' | 'text' | 'console' | 'network' | 'errors';
...
function BrowserPanel() {
  ...
  const [viewTab, setViewTab] = useState<BrowserViewTab>('snapshot');
  ...
  // Sync URL bar when QA navigates
  useEffect(() => {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="border-border bg-card sticky top-0 z-10 flex shrink-0 items-center gap-2 border-b px-3 py-3">
        ...
        <span className="text-primary flex-1 text-sm font-bold">Browser</span>
        <Button
          variant="ghost"
          size="icon-xs"
          ...
          {qaInstances.length === 0 ? (
            ...
            <>
              ...
              <div className="border-border flex shrink-0 flex-wrap border-b">
                {(['snapshot', 'screenshot', 'text', 'console', 'network', 'errors'] as BrowserViewTab[]).map((tab) => {
